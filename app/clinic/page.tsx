...
export default function ClinicDashboard() {
  ...
  async function fetchDashboard() {
    try {
      ...
      const response = await fetch(`/api/clinics/${clinicId}/dashboard?${params.toString()}`);
  ...
  function handlePatientClick(patientCode: string) {
    router.push(`/clinic/patient/${patientCode}?clinic_id=${clinicId}&token=${token}`);
  }
                  ...
                  {data.patients.map((patient) => (
                    ...
                    >
                      <td className="px-4 py-3">
                        <span className={`inline-flex px-2 py-1 rounded-full text-xs font-bold border ${getColorBadge(patient.computed_status.signal_color)}`}>
                          {patient.computed_status.signal_color === 'red' ? '긴급' : 
                           patient.computed_status.signal_color === 'yellow' ? '주의' : '양호'}
                        </span>
                      </td>
