...
export async function findWeeklyResponses(
  ...
  return formData
    ...
    .map(row => ({
      timestamp: row.Timestamp || new Date().toISOString(),
      patient_code: row['환자코드'],
      weight_kg: parseNumber(row['체중(kg)'], 0),
      body_fat_percent: parseNumber(row['체지방률(%)'], undefined),
...
function parseNumber(value: string | number, fallback: number | undefined): number | undefined {
